...
```text
[   100.5 lps ] original line
[   100.5 lps /    85.1 mps ] original line
```

`-f` follows a log file from the end by default, like `tail -f`. Use `-from-start` to read existing content first. `lps` is total lines per second. `mps` is matching lines per second when `-filter` is provided. Rates are rolling averages over the last 10 seconds by default; use `-window [seconds]` to change that.
